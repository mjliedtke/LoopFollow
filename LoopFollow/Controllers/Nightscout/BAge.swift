...
extension MainViewController {
    ...
    func updateBage(data: [bageData]) {
        ...
        if let bageTime = formatter.date(from: lastBageString)?.timeIntervalSince1970 {
            Storage.shared.bageInsertTime.value = bageTime
            ...
            if let formattedDuration = formatter.string(from: secondsAgo) {
                infoManager.updateInfoData(type: .bage, value: formattedDuration, numericValue: secondsAgo / 86400)
            }
        }
    }
}
